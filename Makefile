...
VERSION=$(shell git describe --abbrev=8 --dirty 2>/dev/null || echo v0.1)
WADS=wads
ASCIIDOC=asciidoc
...
FREEDOOM1=$(WADS)/lt1-1.iwad
FREEDOOM2=$(WADS)/lt1-2.iwad
FREEDEMO=$(WADS)/lt1-demo.iwad
FREEDM=$(WADS)/freedm.wad # not part of the LibTech1 project

OBJS=$(FREEDOOM1) $(FREEDOOM2) $(FREEDEMO)
...
deutex-check:
	...
	echo "The full path to deutex can be specified by passing"; \
...
#---------------------------------------------------------
# [LibTech1] freedemo iwad

wadinfo_freedemo.txt : buildcfg.txt subdirs lumps/freedoom.lmp
	$(CPP) -P -DFREEDEMO < $< > $@

$(FREEDEMO): wadinfo_freedemo.txt subdirs
	@mkdir -p $(WADS)
	$(RM) $@
	$(DEUTEX) $(DEUTEX_ARGS) -iwad -build wadinfo_freedemo.txt $@
...
CREDITS-LEVELS.txt: CREDITS-LEVELS.txt
	unix2dos --add-bom --newfile $< $@
...
# [LibTech1] I'm not even going to try to touch the install stuff.
# It WILL end in disaster if I do anything more than make a portable wad file.
ifdef FALSE
...
install-freedm: $(FREEDM) $(HTMLDOCS) manual/freedoom-manual.pdf \
	...
	install -Dm 644 CREDITS.txt CREDITS-LEVELS.txt CREDITS-MUSIC.txt NEWS.html README.html -t "$(target)$(docdir)/freedm"
...
install-freedoom: $(FREEDOOM1) $(FREEDOOM2) $(HTMLDOCS)                 \
	...
	install -Dm 644 dist/io.github.freedoom.Phase2.png \
	                -t "$(target)/share/icons"
	install -Dm 644 CREDITS.txt CREDITS-LEVELS.txt CREDITS-MUSIC.txt NEWS.html README.html \
...
uninstall-freedm:
	$(RM)                                                                   \
	      ...
	      "$(target)$(docdir)/freedm/CREDITS.txt"                           \
	      "$(target)$(docdir)/freedm/CREDITS-MUSIC.txt"                     \
	      "$(target)$(docdir)/freedm/CREDITS-LEVELS.txt"                    \
	      "$(target)$(docdir)/freedm/COPYING.txt"                           \
...
uninstall-freedoom:
	$(RM)                                                                   \
	      ...
	      "$(target)$(docdir)/freedoom/CREDITS.txt"                         \
	      "$(target)$(docdir)/freedoom/CREDITS-MUSIC.txt"                   \
	      "$(target)$(docdir)/freedoom/CREDITS-LEVELS.txt"                  \
	      "$(target)$(docdir)/freedoom/COPYING.txt"                         \
